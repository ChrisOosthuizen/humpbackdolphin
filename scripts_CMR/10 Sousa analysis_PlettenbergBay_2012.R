...
# Plettenberg Bay only 2012 data
...
CH = read.table("./data/2012_input.txt", header = T)
CH = data.matrix(CH)
head(CH)
colnames(CH) <- NULL
head(CH)
...
# RUN MODEL
#------------
JS_mod_plet2012 =  JStype.fit.jags(CR.data.matrix = CH,
                          t_lag = time_lag1,
...
saveRDS(JS_mod_plet2012, file = "./BUGSmodel/JS_sousa_2012_Plettenberg.rds")
# reload with:
#JS_mod_plet2012 <- readRDS("./scripts_CMR/BUGSmodel/JS_sousa_2012_Plettenberg.rds")
...
out = as.data.frame(JS_mod_plet2012$chains_mat)
head(out)

mean(out$Nsuper)   

z <- JS_mod_plet2012$chains_mat[, "Nsuper"]
...
plot(1:17, plot_p$value[1:17], type = "b", col = "black")  # model output

# plot entry
plot_b = mean_all_par %>% 
  dplyr::filter(str_starts(parameter, "rho"))
plot_b
plot(1:17, plot_b$value, type = "b", col = "black", ylim = c(0,0.4))   # model output


niter <- nrow(JS_mod_plet2012$chains_mat)

##################### TRACEPLOTS AND DENSITIES IN THE APPENDIX ######################################
#jpeg("Traceplot_Nsup.jpg", width = 800, height = 600, res = 100)
JS_mod_plet2012$chains_mat[, grepl("Nsuper", colnames(JS_mod_plet2012$chains_mat))] %>% 
  as.data.frame %>% 
