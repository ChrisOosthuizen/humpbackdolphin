...
N <- 90                    # Superpopulation size
phi <- rep(0.95, n.occasions-1)           # Survival probabilities (constant)
b <- c(0.1, 0.2, 0, 0.05, 0.1, 0.05, 0.1, 0.05, 0, 0.05, 0, 0.05, 0.1, 0.05, 0.1, 0)   # Entry probabilities
length(b)
sum(b) # must sum to 1.

#p <- c(0.3, 0.5, 0.9, 0.2, 0.5, 0.9, 0.1, 0.8, 0.5, 0.2, 0.9, 0.9, 0.2, 0.1, 0.9, 0.8)  # Time-varying capture probabilities
p <- c(0.1, 0.21, 0.1, 0.15, 0.2, 0.19, 0.15, 0.15, 0.2, 0.15, 0.16, 0.05, 0.3, 0.1, 0.05, 0.1)  # Time-varying capture probabilities
length(p)
...
saveRDS(JS_mod, file = "./BUGSmodel/results_JS_mod_simulation07_90dolphins.rds")
# reload with:
...
mean(out$Nsuper)   # 90 was the simulated value

hist(out$Nsuper)   # 90 was the simulated value
...
saveRDS(RPT_mod, file = "./BUGSmodel/results_RPT_mod_simulation07_90dolphins.rds")
# reload with:
...
JS_mod$chains_mat[, grepl("Nsuper", colnames(JS_mod$chains_mat))] %>% 
  as.data.frame %>% 
  mutate(Iter = rep(1:(niter/2), times = 2), chain = rep(c(1,2), each = (niter/2)) %>% 
           factor()) %>%
...
JS_mod$chains_mat[, grepl("Nsuper", colnames(JS_mod$chains_mat))] %>% 
  as.data.frame %>% mutate(Iter = rep(1:(niter/2), times = 2), chain = rep(c(1,2), each = (niter/2)) %>% 
                             factor()) %>%
...
##################### TRACEPLOTS AND DENSITIES IN THE APPENDIX ######################################
#jpeg("Traceplot_Nsup.jpg", width = 800, height = 600, res = 100)
RPT_mod$chains_mat[, grepl("Nsuper", colnames(RPT_mod$chains_mat))] %>% 
  as.data.frame %>% 
  mutate(Iter = rep(1:(niter/2), times = 2), chain = rep(c(1,2), each = (niter/2)) %>% 
  factor()) %>%
  ggplot(aes(x = Iter, y = ., color = chain)) + 
