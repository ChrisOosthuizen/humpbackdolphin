...
# 2. Add 260 transients to EHM
# This gives approximately 26 % transients in the data - the same as observed for Indian Ocean humpback dolphins
# Use low capture probabilities - same as observed for Indian Ocean humpback dolphins
# 3. Augment EHM for MCMC
...
b <- c(0.2, 0.05, 0.1, 0, 0.05, 0, 0.3, 0.05, 0, 0.1, 0, 0.05,0.1, 0, 0, 0)   # Entry probabilities
length(b)
sum(b) # must sum to 1.

#p <- c(0.3, 0.5, 0.9, 0.2, 0.5, 0.9, 0.1, 0.8, 0.5, 0.2, 0.9, 0.9, 0.2, 0.1, 0.9, 0.8)  # Time-varying capture probabilities
p <- c(0.2, 0.15, 0.1, 0.2, 0.1, 0.15, 0.3, 0.1, 0.2, 0.15, 0.3, 0.1, 0.2, 0.2, 0.3, 0.1)  # Time-varying capture probabilities
length(p)
...
saveRDS(JS_mod, file = "./BUGSmodel/results_JS_mod_simulation_260transients.rds")
# reload with:
#JS_mod <- readRDS("./BUGSmodel/results_JS_mod_simulation.rds")
...
mean(out$Nsuper)   # 1050 was the simulated value
...
saveRDS(RPT_mod, file = "./BUGSmodel/results_RPT_mod_simulation_260transients.rds")
# reload with:
#JS_mod <- readRDS("./BUGSmodel/results_RPT_mod_simulation.rds")
