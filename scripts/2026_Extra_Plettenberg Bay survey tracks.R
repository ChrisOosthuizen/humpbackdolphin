...
# Plettenberg Bay map with GPS survey lines 
...
lon_div1  <-  23.36463  # Knysna | Plettenberg Bay
...
tracks_lines$track_label <- "Survey tracks"
...
#-----------------------------------
  ...
  tm_lines(
    col = "track_label",
    col.scale = tm_scale_categorical(values = "deeppink"),
    lwd = 1.4,
    col_alpha = 0.2,   # low alpha so overlaps visually accumulate
    col.legend = tm_legend(
      title       = "Survey tracks",
      frame       = FALSE,
      bg.color    = NA,
      position    = tm_pos_in(pos.h = 0.65, pos.v = 0.52),
      text.color  = "white",
      title.color = "white")
  ) + 
...
# 8. Save
# ============================================================
tmap_save(plet_map, "./supplement/Extra_Plettenberg Bay survey tracks.png",
          width  = 6000,
          height = 3000,
          units  = "px",
          dpi    = 600)
